...
export default function TempoEspera() {
  ...
  function carregar(p: Projeto) { setF(p.inputs as Form); setProjetoId(p.id); setNome(p.nome); snapshot.current = JSON.stringify(p.inputs); setSalvoEm(p.atualizadoEm); setEstado("salvo"); }
  function novo() { setF(PADRAO); setProjetoId(null); setNome(""); snapshot.current = ""; setSalvoEm(null); setEstado("nao-salvo"); }
    ...
    <div className="space-y-5">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink-700 bg-ink-900/90 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-2 px-4 py-3">
          <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Nome do projeto (ex.: Casa Jerivá — Espera)" className="min-w-0 flex-1 rounded-xl border border-ink-600 bg-ink-800 px-3 py-2.5 text-sm text-zinc-100 outline-none focus:border-amber/60" />
